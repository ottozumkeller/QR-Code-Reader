...
Scan(*)
{
    ...
    (
        ...
        if ([Windows.Forms.Clipboard]::ContainsImage()) {
            ...
            if ([Windows.ApplicationModel.DataTransfer.Clipboard]::IsHistoryEnabled()) {
                ...
                Set-Clipboard -Value $History::Items[0]
            } else {
                $Image = [Windows.Forms.Clipboard]::GetImage()
                $Image.Save($Name, [Drawing.Imaging.ImageFormat]::Png)
            }
            ...
            if (($Title_String -ne '' -and $Title_String -ne $Null)) {
            ...
            If (([Uri] $Title_String).Scheme -In $Schemes) {
                ...
                If (([Uri] $Title_String).Scheme -Eq 'wifi') {
                    $Array = $Title_String.Split(':').Split(';')
    ...
    ;MsgBox("PowerShell.exe -NoExit -Command &{" . Script . "}")
